...
import { getVimeoEmbedUrl } from '../components/utils';
...
const AirskillPage = ({ data }) => {
  ...
  const videoUrl = idx.videoUrl ? getVimeoEmbedUrl(idx.videoUrl) : null;
  const bodyImageText = idx.bodyImageText?.data?.bodyImageText;
  const body = idx.body.data.body;
  const benefits = idx.benefits || [];
  ...
  return (
    <Layout>
      {/* Hero Section - Replace old header */}
      {idx.hero && (
        <Hero heroID={idx.hero.heroID} allHeroes={allHeroes} />
      )}
      ...
      <div className="container m-auto px-6 py-12 bg-white">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {benefits.map((benefit, index) => {
            ...
            const icon = benefitIcons[iconIndex] || <FaRocket />;
